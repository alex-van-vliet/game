...
TEST_CASE("wrapped functions should automatically throw", "[libsdl]") {
    REQUIRE_THROWS_MATCHES(my_wrapped_ptr(nullptr), std::runtime_error,
                           Catch::Matchers::Message("Correct error was thrown."));

    int i = 2;
    REQUIRE(my_wrapped_ptr(&i) == &i);

    REQUIRE_THROWS_MATCHES(my_wrapped_int(-1), std::runtime_error,
                           Catch::Matchers::Message("Correct error was thrown."));
...
namespace {
    ...
    constexpr TypedWrapper<my_ptr_generator, std::unique_ptr<int>, TestGetError> my_wrapped_ptr_generator;
}

TEST_CASE("typed wrapped functions convert to the given type", "[libsdl]") {
    REQUIRE(std::is_same_v<decltype(my_wrapped_ptr_generator(true, 1)), std::unique_ptr<int>>);
}

TEST_CASE("typed wrapped functions are error checked before transforming the type", "[libsdl]") {
    REQUIRE_THROWS_MATCHES(my_wrapped_ptr_generator(false, 1), std::runtime_error,
                           Catch::Matchers::Message("Correct error was thrown."));
